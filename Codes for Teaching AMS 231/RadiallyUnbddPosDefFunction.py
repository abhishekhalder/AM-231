...
Z_manifold = ((X**2 + Y**2)/(1 + X**2 + Y**2) + (X - Y)**2) + 5
...
plt.xlim((Lx1, Lx2))
plt.ylim(Ly1, Ly2)
a1 = Arrow3D([-0.04, 4.5], [-0.04, -0.04], 
                [4.75, 4.75], mutation_scale=20, 
                lw=1, arrowstyle="-|>", color="r")
ax.add_artist(a1)
a2 = Arrow3D([-0.05, -0.05], [-0.05, 4.5], 
                [4.5, 4.5], mutation_scale=20, 
                lw=1, arrowstyle="-|>", color="r")
ax.add_artist(a2)
ax.axis('tight')
ax.set_aspect('equal')
plt.axis('off')
plt.savefig("PythonContour.png", dpi=300)